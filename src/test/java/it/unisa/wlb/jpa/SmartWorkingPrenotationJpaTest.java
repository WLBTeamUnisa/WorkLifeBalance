...
class SmartWorkingPrenotationJpaTest {
    ...
    void retrieveByWeeklyPlanningTest() {
        ...
        retrieved = prenotationJpa.retrieveByWeeklyPlanning(prenotation.getCalendarWeek(), prenotation.getYear(),
                employee.getEmail());
    ...
    void retrieveAllTest() {
        ...
        list = prenotationJpa.retrieveAll();
        for (SmartWorkingPrenotation aPrenotation : list) {
            if (aPrenotation.getId().getId() == prenotation.getId().getId())
                checkOne = true;
            if (aPrenotation.getId().getId() == secondPrenotation.getId().getId())
                checkTwo = true;
        }
        assertTrue(checkOne);
        assertTrue(checkTwo);
