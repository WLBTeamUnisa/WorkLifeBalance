...
class WorkstationPrenotationJpaTest {
    ...
    void retrieveByEmployeeTest() {
        ...
        for (WorkstationPrenotation aPrenotation : list) {
            if (!aPrenotation.getEmployee().getEmail().equals(employee.getEmail()))
                check = false;
        }
        assertTrue(check);
    ...
    void retrieveByWeeklyPlanningTest() {
        ...
        for (WorkstationPrenotation aPrenotation : list) {
            if (!aPrenotation.getEmployee().getEmail().equals(employee.getEmail())
                    || !(aPrenotation.getCalendarWeek() == calendarWeek))
                check = false;
        }
        assertTrue(check);
    ...
    void retrieveByWorkstationDateTest() {
        ...
        list = prenotationJpa.retrieveByWorkstationDate(workstationPrenotationDate, firstFloor.getNumFloor(),
                firstRoom.getId().getNumRoom());
        for (WorkstationPrenotation aPrenotation : list) {
            if (!(aPrenotation.getId().getPrenotationDate().compareTo(workstationPrenotationDate) == 0)
                    ...
                    || !(aPrenotation.getWorkstation().getRoom().getFloor().getNumFloor() == firstFloor.getNumFloor()))
                check = false;
        }
        assertTrue(check);
    ...
    void retrieveAllTest() {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i).getId().getId() == prenotation.getId().getId())
                check = true;
        }
        assertTrue(check);
