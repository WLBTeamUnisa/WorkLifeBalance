...
class WorkstationJpaTest {
    ...
    void retrieveByFloorAndRoomTest() {
        ...
        for (int i = 0; i < stationsList.size(); i++) {
            if (stationsList.get(i).getId().getWorkstation() == workstation.getId().getWorkstation())
                check1 = true;
            if (stationsList.get(i).getId().getWorkstation() == aWorkstation.getId().getWorkstation())
                check2 = true;
        }
        assertTrue(check1);
        assertTrue(check2);
    ...
    void retrieveById() {
        ...
        retrieved = workstationJpa.retrieveById(firstFloor.getNumFloor(), firstRoom.getId().getNumRoom(),
                workstation.getId().getWorkstation());
