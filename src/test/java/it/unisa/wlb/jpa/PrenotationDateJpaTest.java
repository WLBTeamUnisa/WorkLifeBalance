...
class PrenotationDateJpaTest {
    ...
    final void testUpdate() {
        ...
        try {
            ...
            entityManager
                    .createNativeQuery(
                            "delete from prenotation_date where DATE=? and EMPLOYEE_EMAIL=? and ID_PRENOTATION_SW=?")
                    .setParameter(1, modifyied.getId().getDate()).setParameter(2, modifyied.getId().getEmployeeEmail())
                    .setParameter(3, modifyied.getId().getIdPrenotationSw()).executeUpdate();
            entityManager.getTransaction().commit();
        } finally {
            entityManager.close();
        }
    ...
    final void testRetrieveBySmartWorking() {
        boolean check1 = false;
        boolean check2 = false;
        List<PrenotationDate> list = prenotationDateJpa.retrieveBySmartWorking(
                prenotationDate.getId().getIdPrenotationSw(), prenotationDate.getId().getEmployeeEmail());
