...
public class SmartWorkingDaysPrenotationServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (employee != null) {
            ...
            for (int i = 0; i < arrayDates.length; i++) {
                ...
                if (arrayDates[i] != null && !arrayDates[i].equals("")) {
                    try {
                        ...
                        /**
                         * Checking if currentCalendarWeek is the last week of year and bookingCalendarWeek is the first
                         * of next year
                         */
                        ...
                        } else if (bookingCalendarWeek - 1 == currentCalendarWeek && (localCalendar
                                .get(Calendar.YEAR) == bookingCalendar.get(Calendar.YEAR)
                                || localCalendar.get(Calendar.YEAR) == bookingCalendar.get(Calendar.YEAR) - 1)) {
                        ...
                        RequestDispatcher dispatcher = request
                                .getRequestDispatcher("WEB-INF/SmartWorkingPrenotation.jsp");
                        dispatcher.forward(request, response);
                    }
                }
            }
            ...
            SmartWorkingPrenotation smartWork = smartWorkingDao.retrieveByWeeklyPlanning(
                    smartWorkBooking.getCalendarWeek(), smartWorkBooking.getYear(),
                    smartWorkBooking.getEmployee().getEmail());
            ...
            for (int i = 0; i < dateList.size(); i++) {
                ...
                prenotationDatePK
                        .setDate(Date.from(dateList.get(i).atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
