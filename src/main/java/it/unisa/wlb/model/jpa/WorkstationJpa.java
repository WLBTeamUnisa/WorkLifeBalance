...
public class WorkstationJpa implements IWorkstationDao {
    ...
    public List<Workstation> retrieveByFloorAndRoom(int idFloor, int idRoom) {
        ...
        TypedQuery<Workstation> query = entityManager
                .createNamedQuery("Workstation.retrieveByFloorAndRoom", Workstation.class).setParameter(1, idFloor)
                .setParameter(2, idRoom);
    ...
    public Workstation retrieveById(int idFloor, int idRoom, int idWorkstation) {
        entityManager.getTransaction().begin();
        TypedQuery<Workstation> query = entityManager.createNamedQuery("Workstation.retrieveById", Workstation.class)
                .setParameter(1, idWorkstation).setParameter(2, idFloor).setParameter(3, idRoom);
        entityManager.getTransaction().commit();
    ...
    public int countMaxByFloorAndRoom(int idFloor, int idRoom) {
        entityManager.getTransaction().begin();
        TypedQuery<Long> query = entityManager.createNamedQuery("Workstation.countMaxByFloorAndRoom", Long.class)
                .setParameter(1, idFloor).setParameter(2, idRoom);
        entityManager.getTransaction().commit();
