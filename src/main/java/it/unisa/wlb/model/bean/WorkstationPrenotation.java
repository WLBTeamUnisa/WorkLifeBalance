...
@NamedQueries({ @NamedQuery(name = "WorkstationPrenotation.findAll", query = "SELECT w FROM WorkstationPrenotation w"),
...
public class WorkstationPrenotation implements Serializable {
     ...
     */
    @ManyToOne
    @JoinColumns({ @JoinColumn(name = "NUM_FLOOR", columnDefinition = "int(3)", nullable = false),
            @JoinColumn(name = "NUM_ROOM", columnDefinition = "int(2)", nullable = false),
            @JoinColumn(name = "NUM_WORKSTATION", columnDefinition = "int(3)", nullable = false) })
