...
@NamedQueries({
	...
	@NamedQuery(name="Employee.findByProjectId", query="SELECT e FROM Employee e WHERE e.email = :email and e.password = :password"),
	@NamedQuery(name="Employee.searchByEmail", query="SELECT e FROM Employee e WHERE e.email LIKE CONCAT('%',:email,'%')")
})
